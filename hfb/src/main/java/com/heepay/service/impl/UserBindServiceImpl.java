...
public class UserBindServiceImpl extends ServiceImpl<UserBindMapper, UserBind> implements UserBindService {
	...
	public UserBind bind(Map<String, Object> paramMap) {
		...
		//判断之前是否已经绑定过
		String agentUserId = userBind.getAgentUserId();
		QueryWrapper<UserBind> userBindQueryWrapper = new QueryWrapper<>();
		userBindQueryWrapper.eq("agent_user_id",agentUserId);
		...
		if(userBindDb==null){
			...
			//绑定过,更新userbind
			userBindDb.setPersonalName(userBind.getPersonalName());
			userBindDb.setIdCard(userBind.getIdCard());
			userBindDb.setBankNo(userBind.getBankNo());
			userBindDb.setMobile(userBind.getMobile());
			userBindDb.setPayPasswd(userBind.getPayPasswd());
